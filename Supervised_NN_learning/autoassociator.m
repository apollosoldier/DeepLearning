function [epoch, TR_errors, TR_prop_correct, ...
          TEST_errors] = ... 
          autoassociator(orig_tr_patterns, orig_test_patterns, no_of_hiddens)

%optional parameters : av_category_separation.  Set between 0 and 1
% good sets to demo this:
% tr_mat = 2*round(rand(10,10)) - 1;
% test_mat = [2*round(rand(5,10))-1 ; tr_mat(1:5, :)];   % first half not seen
%                                                        % last five learned before
% no_of_hiddens = 4
...
NO_OF_TEST_PATTERNS = size(orig_test_patterns, 1);  % ditto
...
test_in_pats = orig_test_patterns;
test_target_pats = orig_test_patterns;
...
for p = 1:NO_OF_TEST_PATTERNS
  if TEST_errors(p) <= 0.1
    fprintf('SEEN\n');
  elseif (TEST_errors(p) > 0.1) && (TEST_errors(p) < 0.3)
    fprintf('Possibly Seen \n');
  else
    fprintf('NOT SEEN\n');
  end;
end;
